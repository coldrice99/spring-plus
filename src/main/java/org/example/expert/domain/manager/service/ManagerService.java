...
public class ManagerService {
    ...
    public ManagerSaveResponse saveManager(UserDetailsImpl userDetails, long todoId, ManagerSaveRequest managerSaveRequest) {
        // 매니저 등록 요청 정보
        String logDetails = String.format("User: %s, Todo: %d, ManagerUserId: %d",
                userDetails.getUsername(), todoId, managerSaveRequest.getManagerUserId());
        ...
        try {
            ...
            // 성공 로그 기록
            logService.saveLog(logDetails, "SUCCESS");
            log.info("logDetails: " + logDetails);
            ...
            // 실패 로그 기록
            logService.saveLog(logDetails, "FAILURE");
            throw e;
        }
    }
    ...
    @Transactional
    public void deleteManager(UserDetailsImpl userDetails, long todoId, long managerId) {
        User user = User.fromAuthUser(userDetails);
